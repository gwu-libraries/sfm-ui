...
1. Either `git <https://git-scm.com/>`_ clone the sfm-docker repository and copy the example configuration files::
    ...
    cd sfm-docker
    # Replace 2.0.1 with the correct version.
    git checkout 2.0.1
    cp example.prod.docker-compose.yml docker-compose.yml
    cp example.env .env

or just download ``example.prod.docker-compose.yml`` and ``example.env`` (replacing 2.0.1 with the correct version)::

    curl -L https://raw.githubusercontent.com/gwu-libraries/sfm-docker/2.0.1/example.prod.docker-compose.yml > docker-compose.yml
    curl -L https://raw.githubusercontent.com/gwu-libraries/sfm-docker/2.0.1/example.env > .env
...
statements change *master* to the correct version, e.g., *2.0.1*::
...
otherwise, replacing 2.0.1 with the correct version::
